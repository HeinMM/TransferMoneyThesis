    ...
    public function handle(Request $request, Closure $next)
    {
        if (isset($request) && $request->signature) {
            try {
                ...
                $stringSig = md5($request->agentCode . $request->processIdentifier . $request->old_password . $request->new_password . $request->confirm_password);
